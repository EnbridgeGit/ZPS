REPORT zppsr004 NO STANDARD PAGE HEADING LINE-SIZE 132 LINE-COUNT 65
   MESSAGE-ID zp.
...
************************************************************************
* CHANGES
* 00/11/16 mdemeest 4.6B Titles should print on total lines.
* 00/05/17 mokhan   #753 Print complete heading on each page.
*                   #753 Format intensified not required for detail line
*                   #753 Large amounts appear with indentation.
* 00/05/05 mdemeest #--- Add Person Responsible to each page
* 00/04/10 mdemeest #--- Clear actuals & plans in project_tab
* 00/04/05 mdemeest #--- Expanded value fields
* 00/03/16 mdemeest #753 Copied ZKORR001 as template
* 02/02/08 cmccoy   #- function calls added for set info (see ZPPSR005)
************************************************************************
...
DATA: BEGIN OF project_tab OCCURS 100,
        ...
        pper     LIKE cosp-wkg001,      "Sum of Plans for Period
...
*-----------------------------------------------------------------------
AT SELECTION-SCREEN.
*  if s_pspnr(3) = space and s_vernr(3) = space.
...
TOP-OF-PAGE.
  ...
  ULINE.
  WRITE: /2 vernr,             30 verna.          " Issue # 753
  WRITE: /2 project_tab-pspid, 30 post1.          " Issue # 753
  FORMAT INTENSIFIED ON.
  PERFORM print_headings.                         " Issue # 753
*-----------------------------------------------------------------------
...
FORM get_associated_costs.
  SELECT * FROM cosp
    ...
    IF cosp-wrttp = '01'.                                   "Plans
      ...
      ADD cosp-wkg001 FROM p_start TO p_end GIVING project_tab-pper.
    ...
    IF cosp-wrttp = '04'.                                   "Actuals
      CHECK cosp-versn = 0.
      ADD cosp-wkg001 FROM p_start TO p_end GIVING project_tab-aper.
      MOVE cosp-kstar TO project_tab-kstar.
      COLLECT project_tab.
  ...
  SELECT * FROM coss
    ...
    IF coss-wrttp = '01'.                                   "Plans
      ...
      ADD coss-wkg001 FROM p_start TO p_end GIVING project_tab-pper.
      COLLECT project_tab.
      CLEAR project_tab-pper.
    ENDIF.
    ...
    IF coss-wrttp = '04'.                                   "Actuals
      CHECK coss-versn = 0.
      MOVE coss-kstar TO project_tab-kstar.
      ADD coss-wkg001 FROM p_start TO p_end GIVING project_tab-aper.
      COLLECT project_tab.
      CLEAR project_tab-aper.
    ENDIF.
  ENDSELECT.
  ...
  LOOP AT project_tab.
    LOOP AT name_tab.
      IF ( project_tab-kstar GE name_tab-from AND
        ...
        MOVE wa_setinfo-title  TO project_tab-descript.     "4.6b
        MODIFY project_tab.
      ENDIF.
    ENDLOOP.
  ENDLOOP.
...
*    at new pspnr.
    AT NEW pspid.
      NEW-PAGE.                                             "Project
*     format intensified off.                 " Issue # 753
*     write: /2 vernr,             30 verna.  " Issue # 753
*     write: /2 project_tab-pspid, 30 post1.  " Issue # 753
*     format intensified on.                  " Issue # 753
*     perform print_headings.                 " Issue # 753
*     format intensified on.                  " Issue # 753
    ENDAT.
    ...
    AT END OF kstar.
      FORMAT INTENSIFIED OFF.             "Issue # 753
      SUM.
      delta1 = project_tab-aper - project_tab-pper.
      delta2 = 0.
      IF project_tab-aper <> 0.
        delta2 = delta1 * 100 / project_tab-aper.
      ENDIF.
      PERFORM print_vert.
      ...
      WRITE: (6) project_tab-kstar UNDER text-002,
             ...
             (14) project_tab-aper UNDER text-004,          "Actuals
             (14) project_tab-pper UNDER text-005,          "Plans
             (14) delta1           UNDER text-006,          "Variance
             (12) delta2           UNDER text-007.     "% variance
      FORMAT INTENSIFIED ON.                       "Issue # 753
    ENDAT.
...
*      write:  'Project Total' under text-003,
             (14) project_tab-aper UNDER text-004,     " Issue # 753
             (14) project_tab-pper UNDER text-005,     " Issue # 753
             (14) delta1           UNDER text-006,     " Issue # 753
             (12) delta2           UNDER text-007.
      FORMAT RESET.
      WRITE: /6 sy-uline(127).
    ENDAT.
...
FORM print_headings.
  ...
  WRITE:  7(125) text-002 COLOR COL_NORMAL ON,
          ...
          81 text-005 COLOR COL_NORMAL ON,
          101 text-006 COLOR COL_NORMAL ON,
          120 text-007 COLOR COL_NORMAL ON.
*         72 text-004 color col_normal on,      " Issue # 753
*         88 text-005 color col_normal on,      " Issue # 753
*         104 text-006 color col_normal on,     " Issue # 753
*         120 text-007 color col_normal on.     " Issue # 753
  FORMAT INTENSIFIED ON.
  WRITE: /6(127) sy-uline.
ENDFORM.
...
*---------------------------------------------------------------------*
FORM print_vert.
* write: /6 sy-vline, 19 sy-vline, 68 sy-vline, 84 sy-vline, "Issue #753
*       100 sy-vline, 116 sy-vline, 132 sy-vline.            "Issue #753

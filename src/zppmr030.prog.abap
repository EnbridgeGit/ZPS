...
*        Date: March, 2001.                                           *
*                                                                     *
*  Description: THIS PROGRAM IS CLONED FROM PROGRAM ZPPMR024, IT LOOKS*
*               AFTER FINAL WBS STATUS INSTEAD OF ACTIVE STATUS.      *
*                                                                     *
*               This program produces a listing of projects with no   *
...
FORM PROCESS_PROJECTS.
 ...
 IF SY-SUBRC = 0.
    ...
    IF SY-SUBRC = '0'.
       ...
       CLEAR: MAX_DATE, MAX_TIME.
       PERFORM GET_MAX_DATE.
...
FORM GET_MAX_DATE.
   SELECT MAX( UDATE ) FROM JCDS INTO MAX_DATE    "Select Latest Date
          WHERE OBJNR = PRPS-OBJNR
            AND INACT <> 'X'.                      "Not Inactie
       IF SY-SUBRC <> 0.
          MESSAGE E019   WITH 'NO DATE FOUND'.
       ENDIF.
   SELECT MAX( UTIME ) FROM JCDS INTO MAX_TIME     "Select Latest Time
          WHERE OBJNR = PRPS-OBJNR
            ...
            AND INACT <> 'X'.                      "Not Inactie
       IF SY-SUBRC <> 0.
          MESSAGE E019   WITH 'NO TIME FOUND'.
       ENDIF.
ENDFORM.
